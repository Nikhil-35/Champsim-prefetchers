...
/*
  ...
  */
    ...
    //LFU
    uint32_t minimum = block[set][0].frequency;
    if (way == NUM_WAY) {
	way = 0;
        for (uint32_t way1 = 0; way1 <NUM_WAY; way1++) {
		if ( block[set][way1].frequency < minimum ){
			way = way1;
			minimum = block[set][way1].frequency;
		}
	}
}
...
void CACHE::lru_update(uint32_t set, uint32_t way)
{
    ...
    block[set][way].lru = 0; // promote to the MRU positopn
    block[set][way].frequency++; // 
}
